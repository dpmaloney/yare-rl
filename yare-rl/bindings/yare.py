import os
from ctypes import WinDLL, Structure, CFUNCTYPE, c_float, c_ulong, c_long, c_uint, c_void_p
...
class YareBindings:
    def __init__(self):
        yare_lib = os.path.join(os.path.dirname(os.path.abspath(__file__)), "yare_rust.dll")
        ...
        self.lib.headless_init.argtypes = [TICKFN, c_uint, TICKFN, c_uint]
...
if __name__ == "__main__":
    ...
    i = c_uint(0)
    ptr = yare.headless_init(bot1, i, bot2, i)
    result = -1
    while result < 0:
        yare.headless_update_env(ptr)
        yare.headless_gather_commands(ptr, c_uint(0))
        yare.headless_gather_commands(ptr, c_uint(1))
